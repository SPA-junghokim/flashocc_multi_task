...
from mmcv.cnn import Conv2d, ConvModule, normal_init, caffe2_xavier_init
...
class SimpleBEVEncoder(nn.Module):
    def __init__(self, in_channels):
        super().__init__()
        # self.inc = nn.Conv2d(in_channels, in_channels*2, 3, 1, 1)
        # self.down1 = Down(in_channels*2, in_channels*4)
        # self.down2 = Down(in_channels*4, in_channels*8)
        # self.down3 = Down(in_channels*8, in_channels*8)
        ...
        self.down3 = Down(in_channels, in_channels)
...
class MlvlPointGenerator:
    ...
    def grid_priors(self, featmap_sizes, dtype=torch.float32, device="cuda", with_stride=False):
        ...
        for i in range(self.num_levels):
            priors = self.single_level_grid_priors(featmap_sizes[i], level_idx=i, dtype=dtype, device=device,
...
class MSDeformAttnPixelDecoder(BaseModule):
    ...
    def __init__(
        ...
        for i in range(self.num_input_levels - 1, self.num_input_levels - self.num_encoder_levels - 1, -1):
            input_conv = ConvModule(in_channels[i], feat_channels, kernel_size=1, norm_cfg=norm_cfg, act_cfg=None,
                                    bias=True)
        ...
        #     lateral_conv = ConvModule(in_channels[i], feat_channels, kernel_size=1, bias=self.use_bias,
        #                               norm_cfg=norm_cfg, act_cfg=None)
        #     output_conv = ConvModule(feat_channels, feat_channels, kernel_size=3, stride=1, padding=1,
        ...
        if self.num_cp > 0:
            ...
            for i in range(self.num_cp):
                self.encoder.layers[i] = checkpoint_wrapper(self.encoder.layers[i])
...
class Mask2FormerTransformerEncoder(DeformableDetrTransformerEncoder):
    ...
    def forward(self, query, query_pos, key_padding_mask, spatial_shapes, level_start_index, valid_ratios,
                reference_points, **kwargs):
        """Forward function of Transformer encoder.
