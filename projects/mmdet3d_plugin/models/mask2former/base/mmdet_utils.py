...
def get_nusc_lidarseg_point_coords_dn(mask_pred, gt_binary, voxel_coord, num_points, oversample_ratio, 
        importance_sample_ratio, point_cloud_range, mask_camera, new_gt_idx, padding_mode='border', remove_noise_lidarseg=False, consider_visible_mask=False):
            ...
            if num_rand > 0:
                while True:
                    ...
                    if point_coords.shape[0] >= num_sampled:
                        point_coords = point_coords[:num_sampled]
                        break
            batch_point_coords.extend([point_coords] * new_gt_idx[index].shape[0])
    else:
        for index, gt_bin in enumerate(gt_binary):
            point_coords = voxel_coord[gt_bin.flatten()][:, :3]
            num_lidarseg = min(num_sampled // 2, point_coords.shape[0])
    ...
    new_mask_pred = []
    for i in new_gt_idx:
        new_mask_pred.append(mask_pred[i])
    new_mask_pred = torch.cat(new_mask_pred)
    point_logits = point_sample_3d(new_mask_pred, point_coords[..., [2, 1, 0]], 
                                padding_mode=padding_mode).squeeze(1)
...
def get_nusc_lidarseg_point_coords_dn_2d(mask_pred, gt_binary, voxel_coord, num_points, oversample_ratio, 
        importance_sample_ratio, point_cloud_range, mask_camera, new_gt_idx, padding_mode='border', remove_noise_lidarseg=False, consider_visible_mask=False):
...
def preprocess_occupancy_gt_dn(gt_occ, num_classes, img_metas, non_vis_semantic_voxel=None,
    ...
    if non_vis_semantic_voxel is not None:
        
        non_vis_semantic_voxel = non_vis_semantic_voxel.squeeze(0)
        
        non_vis_stuff_masks_list = []
        non_vis_stuff_labels_list = []
        for label in semantic_labels:
            if label >= num_classes:
                continue
            
            non_vis_stuff_mask = non_vis_semantic_voxel == label
            non_vis_stuff_masks_list.append(non_vis_stuff_mask)
            non_vis_stuff_labels_list.append(label)
        
        # define a class for binary occupancy
        if with_binary_occupancy:
            non_vis_stuff_mask = non_vis_semantic_voxel < 17
            non_vis_stuff_masks_list.append(non_vis_stuff_mask)
            non_vis_stuff_labels_list.append(torch.tensor(num_classes - 1).type_as(non_vis_stuff_labels_list[0]))
            
        assert len(stuff_masks_list) > 0
        non_vis_stuff_masks_list = torch.stack(non_vis_stuff_masks_list, dim=0).long()
        non_vis_stuff_labels_list = torch.stack(non_vis_stuff_labels_list, dim=0).long()
    else:
        non_vis_stuff_labels_list, non_vis_stuff_masks_list = None, None
    
    return stuff_labels, stuff_masks, binary_mask, non_vis_stuff_labels_list, non_vis_stuff_masks_list
